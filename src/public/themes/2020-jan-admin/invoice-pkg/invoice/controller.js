    ...
    controller: function($http, $location, HelperService, $scope, $routeParams, $rootScope, $location) {
        ...
        var dataTable = $('#invoices_list').DataTable({
            ...
            columns: [
                ...
                { data: 'name', name: 'invoices.name', searchable: true },
                { data: 'delivery_time', name: 'invoices.delivery_time', searchable: true },
                { data: 'charge', name: 'invoices.charge', searchable: false },
            ],
        ...
        $('.page-header-content .display-inline-block .data-table-title').html('Invoices <span class="badge badge-secondary" id="table_info">0</span>');
        $('.page-header-content .search.display-inline-block .add_close_button').html('<button type="button" class="btn btn-img btn-add-close"><img src="' + image_scr2 + '" class="img-responsive"></button>');
        $('.page-header-content .refresh.display-inline-block').html('<button type="button" class="btn btn-refresh"><img src="' + image_scr3 + '" class="img-responsive"></button>');
        $('.add_new_button').html(
            '<a href="#!/invoice-pkg/invoice/add" type="button" class="btn btn-secondary" dusk="add-btn">' +
            'Add Invoice' +
            '</a>'
        );
        ...
        //FOR FILTER
        /*$('#invoice_code').on('keyup', function() {
            dataTables.fnFilter();
        });
...
app.component('invoiceForm', {
    ...
    controller: function($http, $location, HelperService, $scope, $routeParams, $rootScope) {
        ...
        fileUpload();
        $http({
            url: laravel_routes['getInvoiceFormData'],
            method: 'GET',
            params: {
            ...
            if (self.action == 'Edit') {
                ...
                if (self.attachment) {
                    $scope.PreviewImage = 'public/themes/' + self.theme + '/img/invoice_logo/' + self.attachment.name;
